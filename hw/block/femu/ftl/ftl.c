...
static struct line *get_next_free_line(struct ssd *ssd)
{
    ...
    curline = QTAILQ_FIRST(&lm->free_line_list);
    if (!curline) {
        printf("FEMU-FTL: Error, there is no free lines left !!!!\n");
        return NULL;
    }
...
static void ssd_init_params(struct ssdparams *spp)
{
    ...
    spp->pg_rd_lat = 80000;
    spp->pg_wr_lat = 2000000;
    spp->blk_er_lat = 6000000;
    spp->ch_xfer_lat = 40000;
...
static uint64_t ssd_advance_status(struct ssd *ssd, struct ppa *ppa, int cmd)
{
    ...
    case NAND_READ:
        ...
        if (lun->next_lun_avail_time >= now) {
            lun->next_lun_avail_time += spp->pg_rd_lat;
        } else {
            lun->next_lun_avail_time = now + spp->pg_rd_lat;
        }
        /* read: then data transfer through channel */
        if (ch->next_ch_avail_time >= now) {
            ch->next_ch_avail_time += spp->pg_rd_lat;
        } else {
            ch->next_ch_avail_time = now + spp->pg_rd_lat;
        }
        lat = ch->next_ch_avail_time - now;
        break;

    case NAND_WRITE:
        /* write: transfer data through channel first */
        if (ch->next_ch_avail_time >= now) {
            ch->next_ch_avail_time += spp->ch_xfer_lat;
        } else {
            ch->next_ch_avail_time = now + spp->ch_xfer_lat;
        }
        ...
        if (lun->next_lun_avail_time >= now) {
            ...
            lun->next_lun_avail_time = now + spp->pg_wr_lat;
        }
        
        lat = lun->next_lun_avail_time - now;
        break;

    case NAND_ERASE:
        /* erase: only need to advance NAND status */
        if (lun->next_lun_avail_time >= now) {
            lun->next_lun_avail_time += spp->blk_er_lat;
        } else {
            lun->next_lun_avail_time = now + spp->blk_er_lat;
        }

        lat = lun->next_lun_avail_time - now;
        break;
    default:
        printf("Unsupported NAND command: %d\n", cmd);
    }
...
static void gc_read_page(struct ssd *ssd, struct ppa *ppa)
{
    ...
    ssd_advance_status(ssd, ppa, NAND_READ);
}
...
static uint64_t gc_write_page(struct ssd *ssd, struct ppa *old_ppa)
{
    ...
    ssd_advance_status(ssd, &new_ppa, NAND_WRITE);
...
static struct line *select_victim_line(struct ssd *ssd)
{
    ...
    if (victim_line->ipc < ssd->sp.pgs_per_line / 8) {
...
static int do_gc(struct ssd *ssd)
{
    ...
    victim_line = select_victim_line(ssd);
    if (!victim_line) {
    ...
    for (ch = 0; ch < spp->nchs; ch++) {
        for (lun = 0; lun < spp->luns_per_ch; lun++) {
            ...
            ssd_advance_status(ssd, &ppa, NAND_ERASE);
        }
    }
...
uint64_t ssd_read(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        if (!mapped_ppa(&ppa) || !valid_ppa(ssd, &ppa)) {
            printf("lpn(%" PRId64 ") not mapped to valid ppa\n", lpn);
            //printf("Invalid ppa,ch:%d,lun:%d,blk:%d,pl:%d,pg:%d,sec:%d\n",
                    //ppa.g.ch, ppa.g.lun, ppa.g.blk, ppa.g.pl, ppa.g.pg, ppa.g.sec);
            continue;
        }
        sublat = ssd_advance_status(ssd, &ppa, NAND_READ);
...
uint64_t ssd_write(struct ssd *ssd, NvmeRequest *req)
{
    ...
    for (lpn = start_lpn; lpn <= end_lpn; lpn++) {
        ...
        curlat = ssd_advance_status(ssd, &ppa, NAND_WRITE);
