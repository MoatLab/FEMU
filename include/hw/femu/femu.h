...
#define TYPE_FEMU "femu"
#define FEMU(obj) OBJECT_CHECK(FemuCtrl, (obj), TYPE_FEMU)
...
typedef struct FemuCXLOps {
    uint16_t (*get_lsa)(struct FemuCtrl *, void *, uint64_t, uint64_t);
    uint16_t (*set_lsa)(struct FemuCtrl *, const void *, uint64_t, uint64_t);
    MemTxResult (*read)(void *, uint64_t, uint64_t *, unsigned, MemTxAttrs);
    MemTxResult (*write)(void *, uint64_t, uint64_t , unsigned, MemTxAttrs);
} FemuCXLOps;
...
typedef struct FemuCtrl {
    ...
    AddressSpace    cxl_as;
    MemoryRegion    cxl_mr; //Memory region for cxl.mem
    FemuCXLOps      cxl_mem_ops;
    QemuMutex       mutex;

    /* Dram buffer properties */
    uint32_t        bufsz;
    uint8_t         rep; /* Replacement 1:LIFO, 2:FIFO */
    uint8_t         prefetch_degree; /* Prefetching stride */
    /* 
    ...
    */
    ...
    /* Cylon CXLSSD: DER cache backend */
    char            *cache_backend_dev;
    uint64_t        cache_bdev_offset;
    uint64_t        cache_hpa_base;
    HostMemoryBackend *cca_dev;
...
static inline bool CXLSSD(FemuCtrl *n)
{
    return (n->femu_mode == FEMU_CXLSSD_MODE);
}
...
/* Defined /hw/femu/backend/dram.c */
