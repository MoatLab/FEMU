...
if [[ $is_mounted == "" ]]; then
    ...
    # huge=always
    sudo mount -t tmpfs -o size=24G,huge=always tmpfs /mnt/tmpfs
fi
...
[[ ! -e /mnt/tmpfs/test1.raw ]] && ./qemu-img create -f raw /mnt/tmpfs/test1.raw 24G
...
sudo x86_64-softmmu/qemu-system-x86_64 \
    ...
    -device nvme,drive=id0,serial=serial0,id=nvme0,namespaces=1,lver=1,lmetasize=16,ll2pmode=0,nlbaf=5,lba_index=3,mdts=10,lnum_ch=4,lnum_lun=8,lnum_pln=2,lsec_size=4096,lsecs_per_pg=4,lpgs_per_blk=512,lbbtable=/media/bbtable.qemu,lmetadata=/media/meta.qemu,ldebug=0 \
