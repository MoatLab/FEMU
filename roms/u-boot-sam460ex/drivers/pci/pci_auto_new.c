...
int pciauto_region_allocate_new(struct pci_region* res, pci_size_t size, pci_addr_t *bar, int mem)
{
	pci_addr_t addr;
    ...
    if ((mem) && (size >= 0x10000)) {
        pciauto_region_align(res, size);
        addr = res->bus_lower;
    }
	else addr = ((res->bus_lower - 1) | (size - 1)) + 1;
			  ...
			  struct pci_region *io)
{
	...
	for (bar = PCI_BASE_ADDRESS_0; bar < PCI_BASE_ADDRESS_0 + (bars_num*4); bar += 4) {
		...
		if (bar_response & PCI_BASE_ADDRESS_SPACE) {
			...
			if (pciauto_region_allocate_new(bar_res, bar_size, &bar_value, 0) == 0) {
    		...
    		if (pciauto_region_allocate_new(bar_res, bar_size, &bar_value, 1) == 0) {
    			/* Write it out and update our limit */
    			pci_hose_write_config_dword(hose, dev, bar, (u32)bar_value);
