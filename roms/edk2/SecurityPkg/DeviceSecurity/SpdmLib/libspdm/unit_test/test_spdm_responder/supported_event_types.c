...
static void libspdm_test_responder_supported_event_types_case1(void **state)
{
    ...
    spdm_response = (spdm_supported_event_types_response_t *)response;

    assert_int_equal(status, LIBSPDM_STATUS_SUCCESS);
    assert_int_equal(spdm_response->header.spdm_version, SPDM_MESSAGE_VERSION_13);
    assert_int_equal(spdm_response->header.request_response_code, SPDM_SUPPORTED_EVENT_TYPES);
    assert_int_equal(spdm_response->header.param1, g_event_group_count);
    assert_int_equal(spdm_response->header.param2, 0);
    assert_int_equal(spdm_response->supported_event_groups_list_len,
                     g_supported_event_groups_list_len);
    for (uint32_t index = 0; index < spdm_response->supported_event_groups_list_len; index++)
    {
        assert_int_equal(((char *)(spdm_response + 1))[index], (char)index);
    }

    libspdm_dump_data(response, response_size);
