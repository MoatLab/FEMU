...
# If you also want to collect Code Coverage in Linux with AFL and lcov, please Install lcov with: sudo apt-get install lcov.
...
echo "start fuzzing in Linux with AFL"
...
do
    ...
    screen -S ${cmds[$i]} -p 0 -X stuff "afl-fuzz -i $fuzzing_seeds/${cmds[$i]} -o $fuzzing_out/${cmds[$i]} ./${cmds[$i]} @@"
